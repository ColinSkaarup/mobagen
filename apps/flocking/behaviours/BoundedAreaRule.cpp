...
glm::vec2 BoundedAreaRule::computeForce(const std::vector<BoidView>& neighborhood, const BoidView& boid) {
  ...
  const float height = displaySize.y > 0.0f ? displaySize.y : 800.0f;

  const glm::vec2 worldBorder(width, height);
  const glm::vec2 zeroVec(0.0f);

  if (boid.position.x > worldBorder.x - (float)desiredDistance) {
    force.x = ((float)desiredDistance / abs(boid.position.x - worldBorder.x)) * -1;
  }

  else if (boid.position.x < zeroVec.x + (float)desiredDistance) {
    force.x = (float)desiredDistance / abs(boid.position.x - zeroVec.x);
  }

  if (boid.position.y > worldBorder.y - (float)desiredDistance) {
    force.y = ((float)desiredDistance / abs(boid.position.y - worldBorder.y)) * -1;
  }

  else if (boid.position.y < zeroVec.y + (float)desiredDistance) {
    force.y = ((float)desiredDistance / abs(boid.position.y - zeroVec.y));
  }
  // end solution
